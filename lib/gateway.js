...
import { requestNeedsLoginRedirect } from './login-redirect.js'
...
export function createGateway(spec) {
  ...
  function handleLoginRedirect(req, res) {
    if (!requestNeedsLoginRedirect(req)) return false
    let target
    try {
      target = typeof spec.loginUrl === 'function' ? spec.loginUrl() : undefined
    } catch {
      return false
    }
  ...
  function proxyHttp(req, res, tls) {
    ...
    if (handleLoginRedirect(req, res)) return

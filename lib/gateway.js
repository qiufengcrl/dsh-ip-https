...
export function createGateway(spec) {
  ...
  const backendPort = () => Number(spec.backendPort())
  ...
  function proxyUpgrade(req, socket, head) {
    ...
    const headers = forwardHeaders(req.headers, port, {
      forwardedFor: req.socket.remoteAddress,
      forwardedProto: spec.tlsActive() ? 'https' : 'http',
    })
  ...
  async function startHttp() {
    ...
    try {
      ...
      if (err && err.code === 'EADDRINUSE') {
        const msg = `port ${httpPort} in use; ACME http-01 and 80→443 disabled until it is free`
